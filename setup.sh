...
if [ ! -f ${GRUB_BACKUP} ]; then
    ...
    if [ "$NODE_TYPE" == "Worker" ]; then
        ...
        sudo sed '/GRUB_CMDLINE_LINUX=/ s/"$/ iommu=pt intel_iommu=on isolcpus=1-20 noht intel_idle.max_cstate=1 idle=poll pcie_aspm=off"/' ${GRUB_BACKUP} | sudo tee /etc/default/grub > /dev/null
        sudo update-grub
        sudo reboot
    fi
fi
